...
| Drive a Reed.co.uk on-site apply modal | `scripts/reed_apply.py <job_id> [<job_id> ...] [--dry]` — course-signup badge check + unverifiable-eligibility-gate refusal before submit | a bespoke cfx script per posting |
| Drive a TotalJobs.com Smart Apply (StepStone) | `sites/totaljobs.com/scripts/tj_apply.py <job_url> [<job_url> ...] [--dry]` — handles both the review-form and one-click flow shapes, course-signup body-language check, role-specific screening-question refusal | manual per-posting cfx calls (the pre-2026-09-25 approach) |
...
| Check a board's LOGIN / apply-session state (is it logged in, or a session wall?) | `sites/_common/scripts/check_login.py <board>` (linkedin/wttj/indeed/seek + reed/csj/guardian/totaljobs/cvlibrary/amazon/nhs) — probes the ACCOUNT page (homepage headers cache a stale "Sign out" and lie); `logged_in`/`wall`/`guest_ok`/`unknown` verdicts | a bespoke `_<board>_probe.py` / inline session-check JS (the forbidden fork — that's why `_reed_probe.py` got written) |
